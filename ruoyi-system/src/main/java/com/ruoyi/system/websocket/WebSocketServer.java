...
public class WebSocketServer {
     ...
     * Lanya定位数据数据服务
     */
    private static ILanyaPositionCurrentService lanyaPositionCurrentService;
...
//        messageQueue.add(materialLog.toString());
     ...
     */
    @Scheduled(cron = "0/2 * * * * ?")
    public void currentPersonPositionHistoryData() throws ParseException {
        DecimalFormat df = new DecimalFormat("#.##");
        ...
        int visitorCount = 0;
        JSONArray dataArray = new JSONArray();
        for (LanyaPositionCurrent current : lanyaPositionCurrents) {
            if (Strings.isEmpty(current.getPersonPhoto())) {
                visitorCount++;
            } else {
                staffCount++;
            }
        ...
        locationData.put("total", lanyaPositionCurrents.size());
        ...
        staffStat.put("ratio", df.format((float) staffCount / lanyaPositionCurrents.size() * 100));
        personTypeStats.add(staffStat);
        ...
        visitorStat.put("ratio", df.format((float) visitorCount / lanyaPositionCurrents.size() * 100));
