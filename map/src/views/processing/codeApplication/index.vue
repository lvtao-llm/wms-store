...
const showAnimatedLine = () => {
  ...
  // 1. 绘制静态路径线（类似高德地图的路线）
  renderRoutePath(optimizedData);

  // 2. 绘制路径点标记
  renderRoutePoints(optimizedData);
  ...
  // 4. 初始化方向箭头动画
  initDirectionArrows(optimizedData);
...
const renderRoutePath = (data) => {
  ...
  if (shouldRenderShadow) {
    ...
    map.value.addOverlay(shadowRoute);
    presetOverlays.value.items.push(shadowRoute);
  }
  ...
  if (shouldRenderBorder) {
    ...
    map.value.addOverlay(borderRoute);
    presetOverlays.value.items.push(borderRoute);
  }
  ...
  map.value.addOverlay(mainRoute);
  presetOverlays.value.items.push(mainRoute);
  ...
  if (data.length <= 1500) {
    ...
    map.value.addOverlay(highlightRoute);
    presetOverlays.value.items.push(highlightRoute);
  }
};
...
const renderRoutePoints = (data) => {
  ...
  const sampleInterval = data.length > 1000 ? 50 : data.length > 500 ? 25 : 10;
  ...
  data.forEach((item, index) => {
    ...
    if (isKeyPoint) {
      ...
      presetOverlays.value.items.push(marker);
    }
  });
};

// 初始化方向箭头动画 - 性能优化版本
const initDirectionArrows = (data) => {
  const BMap = window.BMap;
  const arrows = [];

  // 数据采样 - 如果数据量很大，进行采样
  const sampleInterval = data.length > 1000 ? 10 : data.length > 500 ? 5 : 2;

  // 在路径的关键转折点添加方向箭头
  for (let i = 0; i < data.length - 1; i += sampleInterval) {
    // 每隔一定距离或方向变化较大时添加箭头
    if (i % (sampleInterval * 2) === 0 || isDirectionChanged(data, i, 20)) {
      ...
      const arrowMarker = createDirectionArrow(start, angle);
...
// 创建方向箭头 - 确保箭头不超过路线宽度
const createDirectionArrow = (point, angle) => {
  const BMap = window.BMap;
  const svgStr = `
