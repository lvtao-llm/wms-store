  ...
  methods: {
    ...
    async ready(qt) {
      ...
      const [dataGeoWorld, dataGeoChina] = await Promise.all([
        new FileLoader(loadingManager).loadAsync(
          `${resourceRootAliyun}/geo/world.geojson`
        ),
        new FileLoader(loadingManager).loadAsync(
          `${resourceRootAliyun}/geo/china.geojson`
        ),
        // createEnvEXR(qt, `${resourceRootAliyun}/textures/kloppenheim_06_puresky_2k.exr`)
      ]);
      ...
      qt.assets = {
        models: {},
        geos: {
          world: JSON.parse(dataGeoWorld),
          china: JSON.parse(dataGeoChina),
        },
        textures: {},
      };
      // 创建世界
