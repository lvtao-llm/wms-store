...
export default {
  ...
  methods: {
     ...
     */
    play(m) {
      this.playSpeed = m ? m : 100;
      const speed = m ? m : 100; // 将倍数转换为间隔时间（毫秒）
      this.isPlay = true;

      // 如果已经在播放，先清除旧定时器
      if (this.interval_play) {
        clearInterval(this.interval_play);
        this.interval_play = null;
      }

      // 如果没有设置播放时间，初始化
      console.log(this.meddleTime);
      if (!this.meddleTime) {
        this.meddleTime = this.realTimeRange[0];
      }
      ...
      // 用新速度重新创建定时器
      this.interval_play = setInterval(playStep, speed);
    },
    ...
    drowMark() {
      ...
      this.markTime.forEach((item) => {
        ...
        let sx =
          ...
          this.px_second;
        let ex =
          (new Date(endTime).getTime() - this.firstTime) /
          1000 /
          this.px_second;
