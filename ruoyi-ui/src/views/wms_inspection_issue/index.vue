  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="是否已解决" prop="isResolved">
        <el-select
          v-model="queryParams.isResolved"
    ...
    >
      ...
      <el-table-column label="是否已解决" align="center" prop="isResolved">
        <template slot-scope="scope">
          <dict-tag
            :options="dict.type.wms_issue_is_resolved"
            :value="scope.row.isResolved"
          />
        </template>
      </el-table-column>
      <el-table-column
    ...
    >
      ...
      >
        ...
        <el-form-item label="解决人" prop="resolverId">
          <el-select
            v-model="formResolved.resolverId"
            placeholder="请输入解决人"
          >
            <el-option
              v-for="dict in staffs"
        ...
        <el-form-item label="解决描述" prop="resolveDescription">
          <el-input
            v-model="formResolved.resolveDescription"
            type="textarea"
            placeholder="请输入内容"
          />
        </el-form-item>
        ...
        <el-form-item label="是否已解决" prop="isResolved">
          <el-select
            v-model="formResolved.isResolved"
            placeholder="请选择是否已解决"
          >
            <el-option
              v-for="dict in dict.type.wms_issue_is_resolved"
...
export default {
  ...
  methods: {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          if (formData.issueTime) {
            formData.issueTime = this.parseTime(
              formData.issueTime,
              "{y}-{m}-{d} {h}:{i}:{s}"
            );
          }
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          if (formData.resolveTime) {
            formData.resolveTime = this.parseTime(
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除巡检问题编号为"' + ids + '"的数据项？')
        .then(function () {
    ...
    handleExport() {
      this.download(
        "system/wms_inspection_issue/export",
        {
          ...this.queryParams,
        },
