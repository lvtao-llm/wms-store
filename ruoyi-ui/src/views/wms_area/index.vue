<template>
  <div class="app-container">
    ...
    <camera :key="indexKey" ref="cameraDetail"></camera>
...
import {listWms_device} from "@/api/system/wms_device";
import camera from "./childView/camera.vue";
...
export default {
  ...
  methods: {
    ...
    handleView(row) {
      // this.indexKey++;
      // this.cameras = row.cameras.length ? row.cameras.split(",") : [];
      this.$refs.cameraDetail.openDia(row);
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          this.form.cameras = this.form.cameras ? this.form.cameras.join(",") : "";
