...
export default {
  ...
  data() {
    ...
    return {
      ...
      // 如果转换后轨迹偏移，可以设置为false测试是否接口返回的已经是BD-09坐标
      enableCoordinateConvert: true, // 暂时禁用，测试是否匹配
  ...
  methods: {
    ...
    showAnimatedLine() {
      ...
      this.time_range = [
        {
          beginTime: this.getTime(optimizedData[0].time),
          endTime: this.getTime(optimizedData[optimizedData.length - 1].time),
          bgColor: "green",
          text: "",
        },
      ];

      this.getTimeLineOne = this.getTime(optimizedData[0].time);
    ...
    setupTimeControls(data, characterMarker) {
      const moveCharacterToTime = (targetTime) => {
        ...
        if (targetTimestamp > data[data.length - 1].time) {
          characterMarker.setPosition(
            new BMap.Point(
      ...
      listLanya_device_card_sender_log_by_name_card_type({ key: a }).then(
        (response) => {
          const processItem = (item) => {
            console.log(item);
            // 如果存在轨迹点数据且启用了坐标转换，进行坐标转换
            if (item.trajectoryPoints && this.enableCoordinateConvert) {
              try {
                const trajectoryPoints = JSON.parse(item.trajectoryPoints);
                // 优先使用百度地图API转换，如果不可用则使用本地算法
                if (window.BMap && window.BMap.Convertor) {
                  convertTrajectoryPointsWithBMap(
                    trajectoryPoints,
                    (convertedPoints) => {
                      item.trajectoryPoints = JSON.stringify(convertedPoints);
                      this.selectData.push(item);
                    }
                  );
                } else {
                  const convertedPoints =
                    convertTrajectoryPoints(trajectoryPoints);
                  item.trajectoryPoints = JSON.stringify(convertedPoints);
                  this.selectData.push(item);
                }
              } catch (e) {
                console.warn("解析轨迹点数据失败:", e);
                this.selectData.push(item);
              }
            } else {
              this.selectData.push(item);
    ...
    changePerson(e) {
      ...
      let trajectoryData = JSON.parse(this.queryParams.status.trajectoryPoints);
      this.tableData = this.enableCoordinateConvert
        ? convertTrajectoryPoints(trajectoryData)
        : trajectoryData;
    },
    changeCar() {
      this.queryParams.status = "";
      let trajectoryData = JSON.parse(
    ...
    handleSelectChange(val) {
      ...
      if (this.enableCoordinateConvert) {
        // 优先使用百度地图API转换
        if (window.BMap && window.BMap.Convertor) {
          convertTrajectoryPointsWithBMap(trajectoryData, (convertedData) => {
            this.tableData = convertedData;
            this.processTrajectoryData();
          });
          return;
        } else {
          this.tableData = convertTrajectoryPoints(trajectoryData);
        }
      } else {
        this.tableData = trajectoryData;
      }
    ...
    processTrajectoryData() {
      ...
      this.time_range = [
        {
          beginTime: this.getTime(this.tableData[0].time),
          endTime: this.getTime(this.tableData[this.tableData.length - 1].time),
          bgColor: "green",
          text: "",
        },
      ];
      this.dateRange = [
        this.getTime(this.tableData[0].time),
        this.getTime(this.tableData[this.tableData.length - 1].time),
      ];
      this.startMeddleTime = this.getTime(this.tableData[0].time);
      this.timeIndex++;
