<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="wms_inspection_ruleList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center"/>
      <el-table-column label="主键" align="center" prop="id"/>
      <el-table-column label="规则名称" align="center" prop="name"/>
      <el-table-column label="巡检周期" align="center" prop="cron"/>
      <el-table-column label="巡检时间范围" align="center" prop="timeTolerance"/>
      <el-table-column label="路径点" align="center" prop="pathPoints"/>
      <el-table-column label="巡检点距离" align="center" prop="pointTolerance"/>
      <el-table-column label="按顺序巡检" align="center" prop="sequentialInspection">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_yes_no" :value="scope.row.sequentialInspection"/>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="centerr" class-name="small-padding fixed-width" width="200px">
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="text"
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="巡检周期" prop="cron">
          <el-input v-model="form.cron" placeholder="请输入巡检周期cron执行表达式">
        ...
        <el-form-item label="巡检时间范围" prop="timeTolerance">
          <el-input v-model="form.timeTolerance" placeholder="请输入巡检时间范围">
            <template slot="append">
              <span>分钟</span>
            </template>
          </el-input>
        ...
        <el-form-item label="巡检点距离范围" prop="pointTolerance">
          <el-input v-model="form.pointTolerance" placeholder="请输入巡检点误差">
            <template slot="append">
              <span>米</span>
            </template>
          </el-input>
        ...
        <el-form-item label="按顺序巡检" prop="sequentialInspection">
          <el-select v-model="form.sequentialInspection" placeholder="请选择是否按顺序巡检">
            <el-option
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        sequentialInspection: [
          {required: true, message: "是否按路径点顺序巡检不能为空", trigger: "change"}
        ],
        status: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除巡检规则编号为"' + ids + '"的数据项？').then(function () {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/wms_inspection_rule/export', {
        ...this.queryParams
