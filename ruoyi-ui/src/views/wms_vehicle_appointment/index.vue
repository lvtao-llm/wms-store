  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column
        label="预约时间"
        align="center"
        prop="gatepassAppointmentTime"
        width="180"
      >
        <template slot-scope="scope">
          <span>{{
            parseTime(
              scope.row.gatepassAppointmentTime,
              "{y}-{m}-{d} {h}:{i}:{s}"
            )
          }}</span>
        </template>
      ...
      <el-table-column label="预约物资" align="center" prop="material" />
      <!-- <el-table-column
        label="操作"
        align="center"
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="驾驶员姓名" prop="vehicleDriverName">
          <el-input
            v-model="form.vehicleDriverName"
            placeholder="请输入驾驶员姓名"
          />
        </el-form-item>
        <el-form-item label="驾驶员手机" prop="vehicleDriverPhone">
          <el-input
            v-model="form.vehicleDriverPhone"
  ...
  methods: {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          listWms_vehicle({ vehiclePlateNo: this.form.vehicleId }).then(
            (response) => {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除车辆预约编号为"' + gatepassIds + '"的数据项？')
        .then(function () {
