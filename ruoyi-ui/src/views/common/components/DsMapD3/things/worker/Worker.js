...
import CptWorker from "./CptWorker/CptWorker.vue";
...
export default class Worker extends Marker {
  init() {
    this.option.vm = instantiateComponentVm(CptWorker);
    super.init();
    this.isWorker = true;
  ...
  afterInstantiation() {
    ...
    this.vm.$on("display-trajectory-history", () => {
      ...
      storeCore().mapHistorySearch.target = "worker-" + this.id;
  ...
  async displayTrajectoryReal() {
    ...
    this.thingTrajectoryReal = new TrajectoryRealtime({
      qt,
      arrow: { urlTexture: `${resourceRootAliyun}/projects/dv-stash-3d-dq/textures/trajectory/arrow.blue.png` },
      line: { urlTexture: `${resourceRootAliyun}/projects/dv-stash-3d-dq/textures/trajectory/line.blue.png` }
    });
